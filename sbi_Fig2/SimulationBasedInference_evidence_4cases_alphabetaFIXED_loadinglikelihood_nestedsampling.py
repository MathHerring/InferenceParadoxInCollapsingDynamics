...
# set up parameters for models and number of observations
alptbet_4cases = [[1,1], [1,0], [0,1], [-1,0]] # contains [alpha,beta] for CIR, Vasicek, WF and Bessel
Ntrajlist = [2,4,8,16,32,64,128,256,512,1024,2048,4096,4096*2,4096*2*2]

Ncases = len(alptbet_4cases)
NNtraj = len(Ntrajlist)

Nboot = 1

evidences = np.zeros((Ncases, NNtraj, Nboot))
evidences_error = np.zeros((Ncases, NNtraj, Nboot))
evidences_RW = np.zeros((Ncases, NNtraj, Nboot))
evidences_error_RW = np.zeros((Ncases, NNtraj, Nboot))


#### MAIN LOOP 
#### calculate evidence for general model and for RW model
for i in np.arange(NNtraj): # loop over number of observations
    for j in np.arange(Ncases): # loop over models

        alpha, beta = alptbet_4cases[j]
        nrealiz = Ntrajlist[i]

        for k in np.arange(Nboot): # bootstrap loop
            
            # get observation
            current_observation = get_observation([1., 0.2, alpha, beta, nrealiz])
            ...
            try:
                ...
                evidences[j,i,k] = evi
                evidences_error[j,i,k] = evierr
            except:
                evidences[j,i,k] = np.nan
                evidences_error[j,i,k] = np.nan
            ...
            try:
                ...
                evidences_RW[j,i,k] = evi
                evidences_error_RW[j,i,k] = evierr
            except:
                evidences_RW[j,i,k] = np.nan
                evidences_error_RW[j,i,k] = np.nan
...
evidences_error = np.nanstd(evidences, axis=2)
evidences = np.nanmean(evidences, axis=2)
evidences_error_RW = np.nanstd(evidences_RW, axis=2)
evidences_RW = np.nanmean(evidences_RW, axis=2)

np.savetxt("phdg_logevidence_VS_Ntraj_4cases_SBI_FIXEDbetalp_together_training40004000_tau02_dt0005_start3_Ntraj1000_Nboot1_longer.csv", evidences, delimiter=",")
np.savetxt("phdg_logevidence_VS_Ntraj_4cases_error_SBI_FIXEDbetalp_together_training40004000_tau02_dt0005_Start3_Ntraj1000_Nboot1_longer.csv", evidences_error, delimiter=",")
np.savetxt("phdg_logevidence_VS_Ntraj_4cases_SBI_FIXEDbetalp_RW_together_training40004000_tau02_dt0005_start3_Ntraj1000_Nboot1_longer.csv", evidences_RW, delimiter=",")
np.savetxt("phdg_logevidence_VS_Ntraj_4cases_error_SBI_FIXEDbetalp_RW_together_training40004000_tau02_dt0005_start3_Ntraj1000_Nboot1_longer.csv", evidences_error_RW, delimiter=",")
