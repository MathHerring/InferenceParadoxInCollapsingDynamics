...
theta, x = simulate_for_sbi(simulation_wrapper, proposal=prior,
                             num_simulations=4000, num_workers=1)
...
with open("likelihood_general_Nens1000_Ntrain40004000_dt0005_untiltau02.pkl", "wb") as handle:
...
# Check prior, simulator, consistency
prior_RW, num_parameters_RW, prior_returns_numpy_RW = process_prior(prior_RW)
simulation_wrapper_RW = process_simulator(simulation_wrapper_RW, prior_RW, prior_returns_numpy_RW)
check_sbi_inputs(simulation_wrapper_RW, prior_RW)

# Create inference object. Here, NPE is used.
inference_RW = SNLE(prior=prior_RW)

# generate simulations and pass to the inference object
theta_RW, x_RW = simulate_for_sbi(simulation_wrapper_RW, proposal=prior_RW,
                             num_simulations=4000, num_workers=1)
inference_RW = inference_RW.append_simulations(theta_RW, x_RW)

# train the density estimator and build the posterior
likelihood_estimator_RW = inference_RW.train()


with open("likelihood_RW_Nens1000_Ntrain40004000_dt0005_untiltau02.pkl", "wb") as handle:
    pickle.dump(likelihood_estimator_RW, handle)